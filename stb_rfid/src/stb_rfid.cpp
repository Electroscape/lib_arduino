 ...
 * @return success
 */
bool STB_RFID::RFIDInitI2c(PN532 &reader) {
    ...
    while (true) {
        ...
        if (!versiondata) {
            if (retries > 10) {
                Serial.println(F("  - PN532 startup timed out"));
                return false;
            }
        } else {
            Serial.print(F("  - PN5"));
 ...
 * @return bool return true for valid mifare classic cards
 */
bool STB_RFID::cardDetectI2c(PN532 &reader, uint8_t *uid) {
    uint8_t success;
    uint8_t uidLength;
    success = reader.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength);
    Serial.println(success);
 ...
 * @return false 
 */
bool STB_RFID::cardReadI2c(PN532 &reader, uint8_t data[16], int datablock=1) {
    uint8_t success;
    uint8_t uid[] = {0, 0, 0, 0, 0, 0, 0 };  // Buffer to store the returned UID
    uint8_t uidLength;
    uint8_t keya[6] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
